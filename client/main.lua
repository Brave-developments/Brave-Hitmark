...
local function DisplayRemainingForFrames(sourceEntity, remainingHealth, remainingArmor, frameCount)
    ...
    while frames < frameCount do
        if remainingArmor > 0 then
            DrawText3D(GetEntityCoords(sourceEntity), armorText, Config.ArmorHitColor.r, Config.ArmorHitColor.g, Config.ArmorHitColor.b, armorOffset)
        else
            DrawText3D(GetEntityCoords(sourceEntity), healthText, Config.NormalHitColor.r, Config.NormalHitColor.g, Config.NormalHitColor.b, healthOffset)
        end
        frames = frames + 1
        Wait(0)
    end
